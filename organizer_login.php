...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT * FROM organizer_register WHERE email='$email' AND password='$password'";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {

         $sql1 = "INSERT INTO organizer_login ( email, password) VALUES ('$email', '$password')";
         $result1 = $conn->query($sql1);
        // Redirect to dashboard
         header("Location:organizer_page1.php"); 
         exit();
    } else {
